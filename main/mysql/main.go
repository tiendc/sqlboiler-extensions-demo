...
const (
	connStr = "admin:password@tcp(localhost:3306)/mydb"
)
...
func main() {
	...
	for i := 1; i <= numOrders; i++ {
		...
		allShipments = append(allShipments, &models.Shipment{
			ID:      i,
			Status:  models.ShipmentsStatusNOT_SHIPPED,
			OrderID: orderID,
		})
	}
	...
	print(">>> Expect Error 1390. Got error:", err)
	expectFailed(err)
	...
	_, err = allUsers.UpsertAll(ctx, db, boil.Infer(), boil.Infer())
	print(">>> Expect OK. Got error:", err)
